...
if (typeof(Delicious.BlogBadge)=='undefined') {
    ...
    Delicious.BlogBadge = function() {
        return {
            ...
            DEBUG:           false,
            url_root:        Delicious.URL_ROOT        || 'http://delicious.com',
            post_url_root:   Delicious.POST_URL_ROOT   || 'http://delicious.com/save',
            json_url_root:   Delicious.JSON_URL_ROOT   || 'http://feeds.delicious.com/v2/json/urlinfo',
            static_url_root: Delicious.STATIC_URL_ROOT || 'http://l.yimg.com/hr',
            ...
            fetchBadgeJSON: function() {
                var json_url = this.json_url_root +
                    "?callback=Delicious.BlogBadge.handleBadgeJSON" +
                    "&noCacheIE="+(new Date()).getTime();

                var showposts = false;
                for (hash in this.badges) {
                    if (!this.badges.hasOwnProperty(hash)) continue;
                    var badge = this.badges[hash];
                    json_url += "&hash="+encodeURIComponent(hash);
            ...
            handleBadgeJSON: function(data) {
                ...
                this.log(data);
                this.log(this.badges);

                for (var i=0,rec; rec=data[i]; i++) {
                    var badge = this.badges[rec.hash];
                ...
                for (hash in this.badges) {
                    ...
                    this.buildBadge(badge, {
                        url: badge.url, total_posts: 0, top_tags: []
                    });
                }
            ...
            renderPostButton: function(badge) {
                ...
                return [
                    el('a', {'class':'save-to-link', 'href':post_url, 'title':'Bookmark '+badge.title+' on Delicious'}, [
                        el('span', {'class':'save-to-link-label'}, ['bookmark this on Delicious'])
                    ])
                ];
            },
            ...
            renderBadge: function(badge) {
                ...
                return [
                    ...
                    (total_posts > 0) ?
                        el('a', {'class':'url-link', 'href':this.url_root+'/url/'+data.hash}, [
                    ...
                    (! (total_posts > 0 && top_tags.length > 0) ) ? '' :  [
                        el('div', {'class':'top-tags-container'}, [
                            ...
                            el('ul', {'class':'top-tags'},
                                map(function(t) {
                                    return [ el('li', {}, [
                                        el('a', {'href':this.url_root+'/tag/'+t, 'title':t}, [t])
                                    ]), ' ' ];
                                }, top_tags)
                            )
                        ])
                            ...
                            el('ul', {}, map( function(post) {
                                ...
                                return el('li', {'class':'xfolkentry'}, [
                                    ...
                                    el('div', {'class':'meta'}, [
                                        ...
                                        el('a', {'class':'author', 'href':this.url_root+'/'+post.un}, [post.un]),
                                        (!post.t.length) ? '' : [
                                            el('span', {'class':'meta-label-to'}, [' to ']),
                                            el('ul', {'class':'tags'}, map(function(t) {
                                                return [ el('li', {'class':'tag'}, [
                                                    el('a', {'rel':'tag', 'href':this.url_root+'/'+post.un+'/tag/'+t}, [t])
                                                ]), ' '];
                                            }, post.t))
                                        ],
...
if (!Delicious.BLOGBADGE_MANUAL_MODE) {
    Delicious.BlogBadge.writeBadge("delicious-blogbadge-"+Math.random(), location.href, document.title, {});
